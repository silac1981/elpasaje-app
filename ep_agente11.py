...
â•‘         EL PASAJE 3D STUDIO â€” AGENTE IA                 â•‘
â•‘         ep_agente.py                                     â•‘
â•‘         Corre solo. Detecta patrones. Manda resumen.     â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

CÃ“MO CORRER:
  python ep_agente.py            â†’ anÃ¡lisis completo ahora
  python ep_agente.py silencioso â†’ sin email (solo log)

SE ACTIVA AUTOMÃTICAMENTE a las 20hs (configurado por backup_manager.py programar)
"""
...
CONFIG = {
    ...
    "app_password":   "ndguxauofxtedqrr",   # â† completar con App Password de Google (16 caracteres)
...
def analizar_patrones():
    """El corazÃ³n del agente â€” busca patrones sin supervisiÃ³n."""
    conn = get_conn()
    cur = conn.cursor()
    hallazgos = []
    ...
    if top_margen:
        mejor = top_margen[0]
        hallazgos.append({
            "tipo": "PatrÃ³n de Margen",
            "senal": f"Producto mÃ¡s rentable: {mejor['nombre']}",
            "dato": f"Margen {mejor['margen_pct']}% â€” Marca: {mejor['marca_ep']}",
            "accion": f"Priorizar producciÃ³n de {mejor['nombre']} ante demanda similar",
            "etiquetas": f"margen_alto|{mejor['categoria'].lower().replace(' ','_')}",
            "confianza": 0.9
        })

    # 2. Clientes recurrentes vs Ãºnicos
    ...
    for c in clientes:
        if c["nro_ordenes"] and c["nro_ordenes"] >= 2:
            hallazgos.append({
                ...
                "senal": f"{c['nombre']} tiene {c['nro_ordenes']} Ã³rdenes â€” cliente consolidado",
    ...
    # 3. Ã“rdenes en producciÃ³n con entrega prÃ³xima
    hoy = datetime.now().date()
    proximas = cur.execute("""
        SELECT id_odv, cliente_id, fecha_entrega, estado, notas
        FROM odv_cabecera
        WHERE estado IN ('ProducciÃ³n', 'Confirmado')
        ...
        if p["fecha_entrega"]:
            try:
                ...
                if dias <= 2:
                    hallazgos.append({
                        "tipo": "Alerta Entrega",
                        "senal": f"ODV {p['id_odv']} vence en {dias} dÃ­as",
                        "dato": f"Estado: {p['estado']} â€” Entrega: {p['fecha_entrega']}",
                        "accion": "Verificar avance de producciÃ³n con Fernando",
    ...
    # 4. CategorÃ­as con mÃ¡s demanda
    cats = cur.execute("""
    ...
    if cats:
        top_cat = cats[0]
        hallazgos.append({
            "tipo": "PatrÃ³n de Demanda",
            "senal": f"CategorÃ­a mÃ¡s demandada: {top_cat['categoria']}",
            "dato": f"{top_cat['pedidos']} pedidos â€” ${top_cat['revenue']:,.0f} en revenue",
            "accion": f"Ampliar catÃ¡logo en {top_cat['categoria']} â€” hay demanda probada",
...
def generar_resumen():
    ...
    totales = cur.execute("""
        SELECT 
            COUNT(*) as total_odv,
            SUM(CASE WHEN estado='ProducciÃ³n' THEN 1 ELSE 0 END) as en_produccion,
            SUM(CASE WHEN estado='Entregado' THEN 1 ELSE 0 END) as entregados,
            SUM(CASE WHEN estado='CotizaciÃ³n' THEN 1 ELSE 0 END) as cotizaciones,
...
def enviar_email(totales, odvs, hallazgos):
    if CONFIG["modo_silencioso"] or not CONFIG["app_password"]:
        print("ðŸ“§ Email omitido (modo silencioso o sin contrasena configurada)")
        return
    ...
    odv_html = ""
    for o in odvs:
        color = {"ProducciÃ³n":"#F39C12", "Entregado":"#27AE60",
                 "CotizaciÃ³n":"#0071BC", "Confirmado":"#8E44AD"}.get(o["estado"], "#888")
        odv_html += f"""
        <tr>
            ...
            <td style='padding:6px;'>{o['fecha_entrega'] or 'â€”'}</td>
        </tr>"""
    ...
    <div style='background:#0071BC; color:white; padding:20px; border-radius:8px;'>
        <h2 style='margin:0;'>ðŸ­ El Pasaje 3D Studio â€” Resumen del Agente</h2>
    ...
    <h3>ðŸ¤– Patrones detectados hoy</h3>
    {hallazgos_html if hallazgos_html else '<p>Sin nuevos patrones detectados hoy.</p>'}

    <div style='background:#F5F5F5; padding:15px; border-radius:8px; margin-top:20px;
                font-size:12px; color:#888; text-align:center;'>
        Agente El Pasaje 3D Studio â€” Corriendo automÃ¡ticamente todos los dÃ­as a las 20hs<br>
    ...
    try:
        ...
        msg["Subject"] = f"[El Pasaje] Resumen del Agente â€” {datetime.now().strftime('%d/%m/%Y')}"
        ...
        print(f"âŒ Error al enviar email: {e}")
        print("   â†’ ConfigurÃ¡ el App Password de Google en CONFIG['app_password']")
...
if __name__ == "__main__":
    print(f"\n{'='*55}")
    print(f"  AGENTE EL PASAJE â€” {datetime.now().strftime('%d/%m/%Y %H:%M')}")
    print(f"{'='*55}\n")

    print("ðŸ” Analizando patrones...")
    hallazgos = analizar_patrones()
    print(f"   â†’ {len(hallazgos)} hallazgos registrados en el log")

    print("\nðŸ“Š Generando resumen del dÃ­a...")
    totales, odvs, logs = generar_resumen()
    print(f"   â†’ {totales['total_odv']} ODV | {totales['en_produccion']} en producciÃ³n | ${totales['facturado_total']:,.0f} facturado")

    for h in hallazgos:
        print(f"\n   ðŸ”¸ [{h['tipo']}] {h['senal']}")
        print(f"      ðŸ’¡ {h['accion']}")

    print("\nðŸ“§ Enviando resumen por email...")
    ...
    try:
        ...
        backup_elpasaje("Agente El Pasaje â€” ejecuciÃ³n automÃ¡tica")
    except Exception as e:
        print(f"âš ï¸  Backup omitido: {e}")

    print(f"\n{'='*55}")
    print(f"  Agente finalizado â€” {datetime.now().strftime('%H:%M:%S')}")
    print(f"{'='*55}\n")
