...
CONFIG = {
    ...
    "ELPASAJE": {
        "repo_path":      _ELPASAJE_DIR,
        "branch":         "main",
        "google_drive":   r"C:\Users\ar028883\Google Drive\BACKUP_ELPASAJE",
        "disco_externo":  r"E:\BACKUP_ELPASAJE",
        # Mezcla de archivos y carpetas — _copiar_a_destino maneja ambos
        "archivos_clave": [
            "elpasaje_v2.db",   # DB — no va a git, sí al backup local
            "main.py",
            "backup_manager.py",
            "modules",          # directorio completo
...
def _git_backup(repo_path, branch, motivo):
    """
    Hace add -u (solo trackeados) + commit + push al repo de GitHub.
    ...
    try:
        ...
        # -u: solo archivos ya trackeados, evita stagear temporales no ignorados
        subprocess.run(["git", "add", "-u"], check=True, cwd=repo_path)
...
def _copiar_a_destino(repo_path, destino_base, archivos_clave):
    """
    Copia archivos y carpetas a destino_base/<timestamp>/.
    Soporta tanto archivos individuales como directorios completos.
    """
    try:
        ...
        copiados = []
        for item in archivos_clave:
            origen = Path(repo_path) / item
            if not origen.exists():
                continue
            if origen.is_dir():
                shutil.copytree(origen, destino / item, dirs_exist_ok=True,
                                ignore=shutil.ignore_patterns("__pycache__", "*.pyc", "*.pyo"))
                copiados.append(f"{item}/")
            else:
                shutil.copy2(origen, destino / item)
                copiados.append(item)

        # .env si existe (sin subir a GitHub)
        env_file = Path(repo_path) / ".env"
        if env_file.exists():
            shutil.copy2(env_file, destino / ".env")
            copiados.append(".env")

        return True, f"Copiados {len(copiados)} items → {destino}"
...
def _ejecutar_backup(proyecto_key, motivo="Backup manual"):
    ...
    gd_encontrado = next((r for r in rutas_gdrive if Path(r).parent.exists()), None)
