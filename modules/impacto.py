...
def render():
    ...
    for col, (fondo_id, f) in zip(cols, FONDOS.items()):
        ...
        with col:
            st.markdown(f"<div style='background:#FAF8F3;border-radius:16px;padding:22px;border:1px solid #DCD5C7;border-top:4px solid {f['color']};margin-bottom:16px;'><div style='font-size:1.8rem;'>{f['emoji']}</div><div style='font-family:\"Source Sans 3\",sans-serif;font-size:1rem;font-weight:700;color:#16181F;margin:8px 0 2px;'>{f['nombre']}</div><div style='font-family:\"Bricolage Grotesque\",sans-serif;font-size:1.8rem;font-weight:800;color:{f['color']};letter-spacing:-.03em;'>${recaudado:,.0f}</div><div style='font-family:\"Source Sans 3\",sans-serif;font-size:11px;font-weight:600;color:#74798A;margin-bottom:12px;'>Meta mensual: ${f['meta']:,.0f}</div><div style='background:#E4DECF;border-radius:999px;height:10px;overflow:hidden;'><div style='width:{pct:.0f}%;background:{f['color']};height:100%;border-radius:999px;'></div></div><div style='font-family:\"Source Sans 3\",sans-serif;font-size:11px;color:#74798A;margin-top:6px;'>{'Meta alcanzada!' if faltan == 0 else f'Faltan ${faltan:,.0f}'}</div></div>", unsafe_allow_html=True)
