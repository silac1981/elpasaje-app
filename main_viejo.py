...
LINEAS = {
    ...
    "francisco_sport":  {"nombre": "Sport",            "color": "#34D399", "emoji": "⚽"},
}
...
if menu == "📊 Dashboard Alejandra":
    ...
    # ── Ganancia por producto + materiales ──
...
# ─────────────────────────────────────────────
    ...
    <div class='main-header'>
        ...
        <p>Gestion de materiales e insumos</p>
    </div>
    """, unsafe_allow_html=True)
    mats = cargar_materiales()
...
# ─────────────────────────────────────────────
    ...
    <div class='main-header'>
        ...
        <p>Resumen de actividad por linea de negocio</p>
    </div>
    """, unsafe_allow_html=True)
    df      = cargar_productos()
    ...
    for _, t in tenants.iterrows():
        ...
        with st.expander(f"{cfg['emoji']} {t['name']} — Stock: ${val:,.0f}"):
            cc1, cc2, cc3 = st.columns(3)
            cc1.metric("Valor en Stock",     f"${val:,.0f}")
            cc2.metric("Ganancia Potencial",  f"${gan:,.0f}")
            cc3.metric("Productos",           f"{len(prod)} SKUs")
            if not prod.empty:
                st.dataframe(
